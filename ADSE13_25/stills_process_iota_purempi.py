...
iota {
  ...
  random_sub_sampling {
    ...
    fraction_nproc = 0.5
      .type = float
      .help = fraction of processors that will be assigned master status
  }
}
'''
...
class Script_iota(Script):
  ...
  def run(self):
    ...
    if pre_import:
      ...
      # Wrapper function
      def do_work(i, item_list, master_rank):
        #print ('Hello from rank %d'%i, subset)
        #x = [i,i,i]
        if i in master_ranks:
          idx = master_ranks.index(i)
          if idx + 1 == len(master_ranks):
            end = size
          else:
            end = master_ranks[idx+1]
          dest_rank = i+1
          while dest_rank < end:
            comm.send(item_list,dest = dest_rank, tag=0)
            print ('rank %d sending information to rank %d'%(i,dest_rank))
            dest_rank +=1
        else:
        ...
        for item in item_list:
          ...
          processor.process_datablock(tag, datablock, i, master_rank)
        processor.finalize()
    ...
    if params.mp.method == 'mpi':
      ...
      if rank in master_ranks:
        ...
        def find_master_rank(rank):
          master_rank = master_ranks[-1]
          for x in master_ranks:
            if x < rank:
              master_rank = x
            else:
              return master_rank
    ...
    try:
      if self.params.dispatch.index:
        if self.params.iota.method == 'random_sub_sampling':
          ...
          import math
          for trial in range(math.ceil(self.params.iota.random_sub_sampling.ntrials/math.ceil(size*self.params.iota.random_sub_sampling.fraction_nproc))):
...
if __name__ == '__main__':
  ...
  try:
    print ('This program is unfinished and should not be used till further updates are made. Exiting')
    exit()
    script = Script_iota()
    script.run()
  except Exception as e:
