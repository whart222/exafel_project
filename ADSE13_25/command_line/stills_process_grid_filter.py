    ...
    if True:
      ...
      if self.params.dispatch.index:
        if self.params.iota.method == 'random_sub_sampling':
          ...
          scale_factor_in_px = 0.0 # Number of pixels max displacement
          scale_factor_dist = 0.0 # distance in Z to be max to be perturbed, 0.2 from A.S Brewster et. al (2018) paper
          ...
          for trial in range(self.params.iota.random_sub_sampling.ntrials):
            ...
            # grid search only along z
            #new_OriZ = OriZ - scale_factor_dist + 2*scale_factor_dist*trial/(self.params.iota.random_sub_sampling.ntrials-1)
            new_OriX = OriX
            new_OriY = OriY
            new_OriZ = OriZ + 1.0*scale_factor_dist*trial/(self.params.iota.random_sub_sampling.ntrials-1)
            panel.set_frame(panel.get_fast_axis(), panel.get_slow_axis(), (new_OriX, new_OriY, new_OriZ))
            print ('GRID_XY TRIAL = ',trial, new_OriZ)
            try:
              #from IPython import embed; embed(); exit()
