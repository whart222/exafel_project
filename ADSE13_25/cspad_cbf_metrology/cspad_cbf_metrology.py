...
phil_scope = parse("""
  ...
  panel_filter = None
    ...
    .help = Specify a list of panels to include during refinement. Default (None) is to use \
            all panels.
  panels_to_ignore = [4,5,6,8,9,10,11,12,13]
    .type = ints
    .help = Specify a list of panels to ignore during refinement. Default (None) is ignore no panel \
            During flat refinement, these panels will be brought up to the same distance as the refined panels \
            No internal geometry changes will be made
...
def refine_expanding(params, merged_scope, combine_phil):
  ...
  # --------------------------
  if params.panels_to_ignore is not None:
    panels_to_ignore = params.panels_to_ignore
    for p in params.panels_to_ignore:
      panels_to_ignore.append(p+16)
      panels_to_ignore.append(p+32)
      panels_to_ignore.append(p+48)
    from libtbx import easy_pickle
    print "Filtering out all reflections except those on panels %s"%(", ".join(["%d"%p for p in panels_to_ignore]))
    combined_path = "%s_combined_reflections.pickle"%params.tag
    data = easy_pickle.load(combined_path)
    sel = None
    for panel_id in panels_to_ignore:
      if sel is None:
        sel = data['panel'] == panel_id
      else:
        sel |= data['panel'] == panel_id
    ...
    for i in levels[j]:
      ...
      if params.flat_refinement and params.flat_refinement_with_distance and i > 0:
        ...
        for panel_group in iterate_detector_at_level(detector.hierarchy(), 0, i):
          ...
          if params.panels_to_ignore:
            test = [list(detector).index(panel) in steps[j] for panel in iterate_panels(panel_group) if panel not in panels_to_ignore]
          else:
        ...
        if len(displacements) != len(list(iterate_detector_at_level(detector.hierarchy(), 0, i))):
          for panel_group in iterate_detector_at_level(detector.hierarchy(), 0, i):
            if params.panels_to_ignore:
              test = [list(detector).index(panel) in steps[j] or panel not in panels_to_ignorefor panel in iterate_panels(panel_group)]
            else:
              test = [list(detector).index(panel) in steps[j] for panel in iterate_panels(panel_group)]
