...
data=/net/dials/raid1/sauter/LS49_integ_step4K/int-0-step4K_MPIbatch_0[0][0-9][0-9][0-9][0-4].img.pickle \
...
output.n_bins=10 \
output.prefix=${trial}"
levmar.parameter_flags=Bfactor,Rxy

if [ "${MULTINODE}" == "True" ]; then
  samosa.join ${effective_params}
  samosa.scale ${effective_params}
  exit
fi
exit
